...
void app_main(void)
{
    ...
    ESP_ERROR_CHECK(esp_partition_mmap(partition_mapx, 0, partition_mapx->size, ESP_PARTITION_MMAP_DATA, (const void*)&mapx_ptr, &mapx_handle));
    // Map the partition to data memory
    ESP_ERROR_CHECK(esp_partition_mmap(partition_mapy, 0, partition_mapy->size, ESP_PARTITION_MMAP_DATA, (const void*)&mapy_ptr, &mapy_handle));
    // Map the partition to data memory
    ESP_ERROR_CHECK(esp_partition_mmap(partition_roi, 0, partition_roi->size, ESP_PARTITION_MMAP_DATA, (const void*)&roi_ptr, &roi_handle));
    ...
    ESP_LOGI(TAG,"\nROI BEFORE ERASE left = %d up = %d right = %d down = %d width = %d height %d",roi_ptr[0],roi_ptr[1],roi_ptr[2],roi_ptr[3],roi_ptr[4],roi_ptr[5]);

    int id_y = 1;
    printf("\n BEFORE ERASE mapx with id_y = %d\n",id_y);
    for( int id_x = 0; id_x < 128 ; id_x++)
        printf("%x ", *(mapx_ptr+id_y*1280+id_x));

    id_y = 1;
    printf("\n BEFORE ERASE mapy with id_y = %d\n",id_y);
    for( int id_x = 0; id_x < 128 ; id_x++)
        printf("%x ", *(mapy_ptr+id_y*1280+id_x));
...
return;
    ...
    ESP_LOGI(TAG,"ROI AFTER ERASE left = %x up = %x right = %x down = %x width = %x height %x",roi_ptr[0],roi_ptr[1],roi_ptr[2],roi_ptr[3],roi_ptr[4],roi_ptr[5]);

    id_y = 1;
    printf("\n AFTER ERASE mapx with id_y = %d\n",id_y);
    for( int id_x = 0; id_x < 128 ; id_x++)
        printf("%x ", *(mapx_ptr+id_y*1280+id_x));
    ...
    memset((void*)roi_buf,0xbb,12);
    memset((void*)mapx_buf,0xaa,1280*2);
    memset((void*)mapy_buf,0x55,1280*2);
    ...
    ESP_LOGI(TAG,"ROI AFTER WRITE left = %x up = %x right = %x down = %x width = %x height %x",roi_ptr[0],roi_ptr[1],roi_ptr[2],roi_ptr[3],roi_ptr[4],roi_ptr[5]);

    id_y = 1;
    printf("\n AFTER WRITE mapx with id_y = %d\n",id_y);
    for( int id_x = 0; id_x < 128 ; id_x++)
        printf("%x ", *(mapx_ptr+id_y*1280+id_x));
    ...
    ESP_LOGI(TAG, "\nUnmapped partition mapx from data memory");
